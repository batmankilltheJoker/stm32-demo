...
{
	if(TIM4 == htim->Instance)//判断定时器4//echo
	{
		if(IC_Count.flg == 0)
		{
			//清零定时器计数
			...
			IC_Count.flg = 1;			//设置已经开始
			IC_Count.num_period = 0;		//溢出计数清零
		}
		else
		{
				//获取定时器计数值
			IC_Count.num_cnt= HAL_TIM_ReadCapturedValue(&htim4,TIM_CHANNEL_2);
			distance=(IC_Count.num_cnt+50000*IC_Count.num_period)/ 1000000.0* 340.0 / 2.0 * 100.0;//通过电平时间差，算出distance//
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(TIM4 == htim->Instance)
	{
		if(IC_Count.flg==1)
		{
			if(IC_Count.num_period==0XFFFF)//电平太长了
			{
				IC_Count.flg=0;					//标记成功捕获一次
